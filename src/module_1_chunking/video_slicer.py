...
class VideoSlicer:
    def __init__(self, chunk_duration=4.0, slide_duration=0.5, stride=2.0, face_size=(256, 256), margin_ratio=0.2):
        self.chunk_duration = chunk_duration
        self.stride = stride
        self.slide_duration = slide_duration
        self.face_cropper = AdvancedFaceCropper(face_size=face_size, margin_ratio=margin_ratio)
    ...
    def _create_slides(self, chunk_video_path, chunk_folder, fps):
        ...
        while cap.isOpened():
            ...
            if len(frames) == frames_per_slide:
                # --- CROP FACE CHO SLIDE ---
                slide_faces, slide_landmarks = self.face_cropper.process_slide(frames)
                if slide_faces is not None:
                    self._save_slide(slides_path, slide_id, slide_faces, slide_landmarks)
                # Nếu None (không đủ mặt thật), bỏ qua slide này
                frames = []
                slide_id += 1
        if len(frames) > 0:
            slide_faces, slide_landmarks = self.face_cropper.process_slide(frames)
    ...
    def _save_slide(self, path, s_id, frames, landmarks):
        # Lưu ảnh mặt
        face_file = os.path.join(path, f"slide_{s_id:02d}_faces.npy")
        np.save(face_file, np.array(frames))
        
        # Lưu tọa độ landmark (để Module 2 chỉ việc đọc ra tính toán)
        landmark_file = os.path.join(path, f"slide_{s_id:02d}_landmarks.npy")
        np.save(landmark_file, np.array(landmarks))
