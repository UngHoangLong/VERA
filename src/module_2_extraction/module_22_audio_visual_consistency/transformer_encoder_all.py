...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x_in, x_in_k=None, x_in_v=None):
        ...
        for layer in self.layers:
            ...
            # print(f"x: {x.shape}, softmax_qk: {softmax_qk.shape}, q_norm: {q_norm.shape}, k: {k.shape}, v: {v.shape}")
...
if __name__ == '__main__':
    ...
    print(f"encoder(x) shape: {x1[-1].shape}, softmax_qk: {softmax_qk1[-1].shape}, q1: {q1[-1].shape}, k1: {k1[-1].shape}, only v1: {v_norm1[-1].shape}")

    small_encoder = TransformerEncoder(256, 8, 4)
    # x = torch.tensor(torch.rand(20, 2, 300))
    x2 = torch.rand(20, 2, 256)
    x2, softmax_qk2, q2, k2, v_norm2, v2 = small_encoder(x2)
    print(f"encoder(x) shape: {x2[-1].shape}, softmax_qk: {softmax_qk2[-1].shape}, q1: {q2[-1].shape}, k1: {k2[-1].shape},  only v2: {v_norm2[-1].shape}")
